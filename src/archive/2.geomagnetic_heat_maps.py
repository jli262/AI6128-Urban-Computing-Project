...
class FloorPlan(RootDirectory):
    def __init__(self, site_number: int, floor: str,data_directory: str = r'data') -> None:
        super().__init__()
    ...
    ## todo
    def plot_magnetic(self, colorbar_title='dBm'):
        mwi_datas = self.calibrate_magnetic_wifi_ibeacon_to_position()
        magnetic_strength = self.extract_magnetic_strength(mwi_datas)
    ...
    # todo
    def calibrate_magnetic_wifi_ibeacon_to_position(self):
        mwi_datas = {}
        ...
        for file in datafiles:
            ...
            if ibeacon_datas.size != 0:
                ...
                for ibeacon_ds in ibeacon_datas_list:
                    ...
                    target_xy_key = tuple(step_positions[index, 1:3])
                    if target_xy_key in mwi_datas:
                        mwi_datas[target_xy_key]['ibeacon'] = np.append(mwi_datas[target_xy_key]['ibeacon'],
                                                                        ibeacon_ds,
                                                                        axis=0)
                    else:
                        mwi_datas[target_xy_key] = {
            ...
            for magn_ds in magn_datas_list:
                ...
                target_xy_key = tuple(step_positions[index, 1:3])
                if target_xy_key in mwi_datas:
                    mwi_datas[target_xy_key]['magnetic'] = np.append(mwi_datas[target_xy_key]['magnetic'], magn_ds,
                                                                     axis=0)
                else:
                    mwi_datas[target_xy_key] = {
                        'magnetic': magn_ds,
...
class DataFile(object):
    ...
    def parse(self):
        ...
        return ReadData(self.acce, self.acce_uncali, self.gyro, self.gyro_uncali, self.magn, self.magn_uncali,
                        self.ahrs, self.wifi, self.ibeacon, self.waypoint)
